extern "C" {
    ...
    static void gen_chunk_backrooms(int slot, int cx, int cy, int seed) {
        ...
        for (int t = 0; t < tcount && ent_count < MAX_ENTS; t++) {
            ...
            float thp = 80.0f + (float)(current_depth * 12);
    ...
    void enter_backrooms() {
        ...
        global_seed = (int)((unsigned int)(global_seed * 2246822519u ^ 0xBACC0000) | 1u);// or just split it
    ...
    void update_logic(float px, float py, int mov, int sprint) {
        if (mov && p_st > 0.0f) {
            p_st -= sprint ? 0.35f : 0.10f;
        } else if (p_st < 100.0f) {
            p_st += 0.30f + st_upgrade * 0.08f;
        }
    ...
    int fire_weapon(float px, float py, float pa) {
        for (int i = 0; i < ent_count; i++) {
            ...
            if (__builtin_fabsf(ang) < 0.35f && dist < 6.0f) {
                e->hp -= 20.0f;
                int spawned = 0;
                ...
                if (e->hp <= 0.0f) {
                    e->active = 0;
                    p_xp    += (e->type == 6) ? 150 : calculate_enemy_xp(e->type, current_depth);
                    p_scrap += (e->type == 6) ? 5   : (e->type == 2) ? 3 : 1;
                }
                return 1;
            }
        }
    ...
    __attribute__((visibility("default"))) int   get_tile_world(int x,int y) { return get_tile_wc(x,y); }
